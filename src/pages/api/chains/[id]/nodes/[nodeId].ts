...
export const PUT: APIRoute = async ({ params, request, locals }) => {
  ...
  if (!id || !nodeId) return new Response(JSON.stringify({ error: 'Not found' }), { status: 404 });

  const owner = await assertChainOwner(locals, id);
  ...
  try {
    const node = await chainRepo.updateNode(locals.supabase, nodeId, parsed.data);
...
export const DELETE: APIRoute = async ({ params, locals }) => {
  if (!locals.user) return new Response(JSON.stringify({ error: 'Unauthorized' }), { status: 401 });
  const { id: deleteId, nodeId: deleteNodeId } = params;
  if (!deleteId || !deleteNodeId)
    return new Response(JSON.stringify({ error: 'Not found' }), { status: 404 });

  const owner = await assertChainOwner(locals, deleteId);
  if (!owner) return new Response(JSON.stringify({ error: 'Not found' }), { status: 404 });
  if (owner === 'forbidden')
    return new Response(JSON.stringify({ error: 'Forbidden' }), { status: 403 });

  try {
    await chainRepo.deleteNode(locals.supabase, deleteNodeId);
