...
export const POST: APIRoute = async ({ params, request, locals }) => {
  ...
  if (!postId) return new Response(JSON.stringify({ error: 'Not found' }), { status: 404 });

  const owner = await assertChainOwner(locals, postId);
  ...
  try {
    const node = await chainRepo.addNode(locals.supabase, postId, parsed.data);
...
export const PUT: APIRoute = async ({ params, request, locals }) => {
  ...
  const owner = await assertChainOwner(locals, putId);
  if (!owner) return new Response(JSON.stringify({ error: 'Not found' }), { status: 404 });
  if (owner === 'forbidden')
    return new Response(JSON.stringify({ error: 'Forbidden' }), { status: 403 });
  ...
  try {
    await chainRepo.reorderNodes(locals.supabase, putId, parsed.data.ordered_ids);
