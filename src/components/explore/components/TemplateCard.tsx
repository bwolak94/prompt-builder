...
export const TemplateCard: React.FC<TemplateCardProps> = ({ template, onFork, lang }) => {
  const { t } = useI18n(lang);

  const difficultyLabels: Record<string, string> = {
    beginner: t('templateCard.difficulty.beginner'),
    intermediate: t('templateCard.difficulty.intermediate'),
    advanced: t('templateCard.difficulty.advanced'),
  };
  ...
  return (
    <article className="group relative flex flex-col rounded-xl border border-border bg-surface-raised p-4 transition-all hover:border-brand-500/40 hover:shadow-sm">
      ...
      <h2 className="mb-1 text-sm font-semibold text-text-primary">
        {(lang === 'en' ? template.title_en : null) ?? template.title}
      </h2>
      <p className="line-clamp-2 text-xs text-text-muted">
        {(lang === 'en' ? template.description_en : null) ?? template.description}
      </p>
      ...
      <div className="mt-4 flex items-center justify-between border-t border-border pt-3">
        <span className="flex items-center gap-1 text-xs text-text-muted">
          ...
          {template.fork_count} {t('templateCard.forks')}
        </span>
        ...
        <button
          ...
          aria-label={`${t('templateCard.use')}: ${template.title}`}
        >
          {t('templateCard.use')} <ChevronRight size={12} aria-hidden="true" />
        </button>
      </div>
    </article>
  );
