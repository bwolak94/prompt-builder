...
export const communityFeedRepo = {
  async findFeed(
    ...
    let query = supabase
      ...
      .select('id,user_id,title,description,tags,fork_count,view_count,avg_rating,rating_count,comment_count,is_featured,created_at,updated_at,trending_score,author_name,author_avatar');
    ...
    if (hasMore && items.length > 0) {
      ...
      switch (filters.tab) {
        case 'trending': nextCursor = String(last.trending_score); break;
        case 'recent':   nextCursor = last.created_at; break;
        case 'top_rated': nextCursor = String(last.avg_rating); break;
        case 'featured': nextCursor = last.updated_at; break;
      }
    }
